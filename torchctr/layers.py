...
class EmbeddingLayer(nn.Module):
    ...
    def __init__(self, x, emb_szs_dict=None, emb_drop=0., mode='mean'):
        ...
        if x.sparse_data:
            ...
            self.sparse_embeds = nn.ModuleList([nn.Embedding(ni, nf) for ni, nf in zip(nuniques, emb_szs)])
    ...
    def emb_sz_rule(self, dim: int) -> int:
        return min(600, round(1.6 * dim**0.56))

    def forward(self, x):
        out = []
        if x.sparse_data:
            data = torch.LongTensor(x.sparse_data.data, device=defaults.device)
            sparse_out = [e(data[:, i]) for i, e in enumerate(self.sparse_embeds)]
            sparse_out = torch.cat(sparse_out, 1)
            sparse_out = self.drop(sparse_out)
            out.append(sparse_out)
        if x.sequence_data:
            data = [torch.LongTensor(t, device=defaults.device) for t in x.sequence_data.data]
            offset = [torch.LongTensor(t, device=defaults.device) for t in x.sequence_data.bag_offsets]
            sequence_out = [e(data[i], offset[i]) for i, e in enumerate(self.sequence_embeds)]
            sequence_out = torch.cat(sequence_out, 1)
            sequence_out = self.drop(sequence_out)
            out.append(sequence_out)
        if x.dense_data:
            dense_data = torch.as_tensor(x.dense_data.data, device=defaults.device)
